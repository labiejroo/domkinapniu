...
export const Okolica = () => {
  ...
  const nearbyPlaces = t.raw('nearbyPlaces') as NearbyPlace[];
  return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.4fr] gap-12 lg:gap-24 items-start">
          ...
          <div>
            <div className="flex justify-between items-baseline border-b border-white/18 pb-3 mb-1">
              <SmallCap className="text-white/50">{t('placeHeader')}</SmallCap>
              <SmallCap className="text-white/50">{t('distanceHeader')}</SmallCap>
            </div>
            {nearbyPlaces.map((row, i) => (
              <div
                key={i}
                className="grid gap-4 md:gap-6 items-baseline py-5 md:py-6 border-b border-white/12 transition-[padding] duration-200 hover:pl-2 group"
